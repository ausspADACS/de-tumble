...
momInertiaMatrix = [0.0612 0 0; 0 0.1346 0; 0 0 0.1758]; % kg * m^2
conv_to_rad = 0.01745329251;
conv_to_deg = 57.29577951;
A = 0.011356; % Area of magnetorquer, m^2
N = 160; % Number of turns on magnetorquer
resistance = 6.4; %ohms
...
k = -25000; %B-dot gain value for de-tumble
...
inclin = 55; %orbital inclination, degrees
...
for j = 1:size(B_field_orb, 1)
    ...
    mag_moment(j,:) = k*cross(B_field_body(j,:),(ang_vel(j,:)*conv_to_rad)); %Teslas X rad/sec
...
current = mag_moment/(A*N); % amps
power = resistance * current.^2; % watts
total_power = sum(sum(power, 1));
energy_per_magnetorquer = power*time_step;
total_energy = sum(sum(energy_per_magnetorquer,1)); % Energy consumed in Joules
...
figure()
subplot(2,1,2)
plot(total_time, power)
xlim([0 1300])
title('Power Consumption')
legend('Px',  'Py', 'Pz')
xlabel('Time, minutes')
ylabel('Power, Watts')
ener_string = ['Total Energy Consumed: ',num2str(total_energy/1000),' kJ'];
text(600,.11,ener_string);
grid on

fprintf('Energy Consumed: %.2f kJ\n', total_energy/1000)
